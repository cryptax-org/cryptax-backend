...
@Controller
open class TransactionRoutes(private val transactionController: TransactionController, private val service: SecurityContextManager) {

    @Post("/users/{userId}/transactions")
    open fun addTransaction(
        @Body @Valid transactionWeb: TransactionWeb,
        @PathVariable userId: String): Single<TransactionWeb> {
        return service.verifyUserId(userId).flatMap { transactionController.addTransaction(userId, transactionWeb) }
    }
    ...
    open fun addMultipleTransactions(
        ...
        return service.verifyUserId(userId).flatMap { transactionController.addMultipleTransactions(userId, transactions) }
    }

    @Get("/users/{userId}/transactions")
    fun getAllTransactions(@PathVariable userId: String): Single<List<TransactionWeb>> {
        return service.verifyUserId(userId).flatMap { transactionController.getAllTransactions(userId) }
    }
    ...
    fun getOneTransaction(
        ...
        return service.verifyUserId(userId).toMaybe().flatMap { transactionController.getTransaction(transactionId, userId) }
    }
    ...
    open fun updateTransaction(
        ...
        return service.verifyUserId(userId).flatMap { transactionController.updateTransaction(transactionId, userId, transactionWeb) }
    }
    ...
    fun deleteTransaction(
        ...
        return service.verifyUserId(userId).flatMap { transactionController.deleteTransaction(transactionId, userId) }
    }
